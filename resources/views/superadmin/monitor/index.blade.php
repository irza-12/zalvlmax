<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div
                ...
                <span>{{ $activeSessions->count() }} user aktif</span>
            </div>
        </div>
    </x-slot>
    ...
    @if($activeQuizzes->count() > 0)
        <div class="mb-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            @foreach($activeQuizzes as $quiz)
                <a href="{{ route('superadmin.monitor.quiz', $quiz) }}"
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="flex items-center justify-between">
                            <p class="text-sm font-medium text-secondary-900">{{ $quiz->title }}</p>
                        ...
                        <div class="flex items-center gap-1 mt-1 text-sm text-secondary-500">
                            ...
                            <span class="font-bold text-secondary-900">{{ $quiz->active_count }}</span> peserta aktif
                        </div>
                    </div>
                </a>
            @endforeach
    ...
    <div class="rounded-lg bg-white shadow-card border border-secondary-100 overflow-hidden">
        ...
        <div id="sessionListContainer">
            @include('superadmin.monitor._session_list')
        </div>
    </div>
    ...
    <script>
        ...
        function refreshMonitor() {
            fetch("{{ route('superadmin.monitor.index') }}", {
                headers: {
