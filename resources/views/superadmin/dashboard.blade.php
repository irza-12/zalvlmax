    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        ...
        <div
            ...
            <dd class="ml-16 flex items-baseline pb-1 sm:pb-2">
                <p class="text-2xl font-bold text-secondary-900">{{ number_format($stats['total_users']) }}</p>
            </dd>
        </div>
        ...
        <div
            ...
            <dd class="ml-16 flex items-baseline pb-1 sm:pb-2">
                <p class="text-2xl font-bold text-secondary-900">{{ number_format($stats['total_quizzes']) }}</p>
            </dd>
        </div>
        ...
        <div
            ...
            <dd class="ml-16 flex items-baseline pb-1 sm:pb-2">
                <p class="text-2xl font-bold text-secondary-900">{{ number_format($stats['attempts_today']) }}</p>
                <span
                    class="ml-2 text-sm font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">+{{ $stats['active_sessions'] }}
                    active</span>
            </dd>
        </div>
        ...
        <div
            ...
            <dd class="ml-16 flex items-baseline pb-1 sm:pb-2">
                <p class="text-2xl font-bold text-secondary-900">{{ $stats['avg_score'] }}%</p>
            </dd>
        </div>
    </div>
                                    ...
                                    <div class="relative flex space-x-3">
                                        <div>
                                            <span class="h-8 w-8 rounded-full flex items-center justify-center ring-8 ring-white 
                                                @if($activity->action === 'create') bg-green-500 
                                                @elseif($activity->action === 'update') bg-blue-500 
                                                @elseif($activity->action === 'delete') bg-red-500 
                                        ...
                                        <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                            <div>
                                                <p class="text-sm text-secondary-500">
                                                    <span
                                                        class="font-medium text-secondary-900">{{ $activity->user->name ?? 'System' }}</span>
        ...
        <div class="space-y-8">
            <div class="bg-white rounded-2xl shadow-card border border-secondary-100 overflow-hidden">
                ...
                <div class="divide-y divide-secondary-100">
                    @forelse($topQuizzes as $index => $quiz)
                        <div class="px-6 py-4 hover:bg-secondary-50/50 transition-colors">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-xs text-secondary-500">{{ $quiz->attempt_count ?? 0 }} attempts</span>
                            </div>
                            ...
                            <div class="flex items-center justify-between text-xs text-secondary-500 mt-3">
                                <span>Avg Score</span>
                                <span
                                    class="font-bold {{ ($quiz->avg_score ?? 0) >= 70 ? 'text-green-600' : 'text-amber-600' }}">
                                    {{ round($quiz->avg_score ?? 0, 1) }}%
                                </span>
                            </div>
                            <!-- Simple bar -->
                            <div class="mt-2 h-1.5 w-full bg-secondary-100 rounded-full overflow-hidden">
                                <div class="h-full bg-brand-500 rounded-full" style="width: {{ $quiz->avg_score ?? 0 }}%">
                                </div>
                            </div>
                        </div>
                    @empty
            ...
            <div
                ...
                <div class="p-6 relative z-10">
                    ...
                    <div class="space-y-3">
                        <a href="{{ route('superadmin.quizzes.create') }}"
